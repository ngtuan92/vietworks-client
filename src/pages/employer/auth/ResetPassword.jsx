...
const EmployerResetPassword = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <main className="w-full max-w-md bg-white border border-slate-200/60 premium-shadow rounded-2xl transition-all shadow-sm p-8">
        <Link to="/" className="text-xl font-bold text-primary">VietWorks</Link>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <button type="submit" disabled={submitting} className="w-full rounded-xl bg-primary text-white py-3 font-semibold disabled:opacity-50">
  ...
  <>
    <div>
      ...
      <div className="relative">
        <input id="password" type={showPassword ? 'text' : 'password'} required value={formData.password} onChange={handleChange} className="w-full rounded-xl border border-slate-200 px-4 py-3 pr-11 outline-none focus:border-primary" placeholder="Ít nhất 8 ký tự" />
    ...
    <div>
      ...
      <input id="confirmPassword" type={showPassword ? 'text' : 'password'} required value={formData.confirmPassword} onChange={handleChange} className="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-primary" placeholder="Nhập lại mật khẩu mới" />
    </div>
  </>
);
