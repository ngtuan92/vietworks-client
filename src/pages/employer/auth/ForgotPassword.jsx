  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-100 to-slate-50 flex items-center justify-center p-4">
      <main className="w-full max-w-xl bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
        <Link to="/" className="text-2xl font-extrabold text-primary">VietWorks</Link>
        ...
        {submitted ? (
          <div className="mt-8 rounded-2xl bg-emerald-50 text-emerald-800 p-5">
            ...
            <Link to="/employer/login" className="inline-flex mt-4 px-5 py-3 rounded-xl bg-primary text-white font-semibold">Quay lại đăng nhập</Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            <div>
              ...
              <input id="email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="w-full rounded-xl border border-slate-200 px-5 py-4 outline-none focus:border-primary" placeholder="hr@company.com" />
            </div>
            <button type="submit" disabled={submitting} className="w-full rounded-xl bg-primary text-white py-4 font-semibold disabled:opacity-50">
